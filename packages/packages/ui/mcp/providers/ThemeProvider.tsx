...
export function McpThemeProvider({
  ...
  const { processedOverrides, hasOverridesChanged, validationResult } =
    useMemo(() => {
      // Note: performance tracking moved outside useMemo to avoid impure function calls
      ...
      // Note: telemetry updates moved to useEffect to avoid setState in render
      ...
      setMetadata((prev) => ({
        ...
        tenantOverrides: Object.keys(filteredOverrides).filter(
