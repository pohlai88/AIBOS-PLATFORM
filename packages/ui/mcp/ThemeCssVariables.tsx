...
import { getVariableBatcher } from "./VariableBatcher";
...
export function McpCssVariables({
  ...
  const { overrides, loading, error } = useMcpTheme({ tenant, safeMode, enabled });
  ...
  useEffect(() => {
    ...
    if (error) {
      ...
      emitTelemetry({
        ...
        error: error.message,
        overrideCount: 0,
        duration: Date.now() - updateStartTimeRef.current,
      });
    ...
    try {
      ...
      const duration = Date.now() - updateStartTimeRef.current;
      emitTelemetry({
        success: true,
  ...
  }, [overrides, enabled, loading, error, batcher, tenant, safeMode, enableTelemetry, showLoadingState]);
...
export function McpThemeDebugger({
  ...
  const { overrides, loading, error } = useMcpTheme({ tenant, safeMode, enabled: true });
