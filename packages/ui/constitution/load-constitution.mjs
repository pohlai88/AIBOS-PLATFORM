...
export function loadConstitution(name) {
  const constitutionPath = path.join(__dirname, `${name}.yml`)
  ...
  try {
    ...
    if (error instanceof yaml.YAMLError) {
      throw new Error(`Failed to parse ${name}.yml: ${error.message}`)
    }
    throw error
  }
}
...
export function loadConstitutionIndex() {
  ...
  if (!fs.existsSync(indexPath)) {
    throw new Error(`Constitution index not found: ${indexPath}`)
  }
...
export function getValidatorForConstitution(constitutionName) {
  const index = loadConstitutionIndex()
  const constitution = index.constitutions.find(
    c => c.name === constitutionName
  )
