...
export async function runValidationPipeline(indexPath = null, args = {}) {
  const indexPathResolved = indexPath || path.join(
  ...
  const fileContent = args.code || args.content || (filePath ? fs.readFileSync(filePath, "utf8") : null);
  ...
  for (const step of index.pipeline) {
    ...
    try {
      ...
      const validateFn = validatorModule.validate ||
                        validatorModule[`validate${step.charAt(0).toUpperCase() + step.slice(1)}`] ||
 ...
 */
export async function runAllValidations(filePath, fileContent = null, options = {}) {
  const {
    skipA11y = false,
    skipToken = false,
    skipRSC = false,
