...
export async function validateMotion(filePath, fileContent = null) {
  ...
  const budgetResult = validateAnimationBudget(
    filePath,
    fileContent,
    motionRules
  )
  ...
  const reducedMotionResult = validateReducedMotion(
    filePath,
    fileContent,
    motionRules
  )
  ...
  const guardrailsResult = validateAnimationGuardrails(
    filePath,
    fileContent,
    motionRules
  )
...
function validateAnimationBudget(filePath, fileContent, rules) {
  ...
  const animationCount = (
    fileContent.match(/(?:animation|transition):/gi) || []
  ).length
...
function validateReducedMotion(filePath, fileContent, rules) {
  ...
  const hasReducedMotion =
    fileContent.includes('prefers-reduced-motion') ||
  ...
  const hasAnimations =
    fileContent.includes('animation:') ||
  ...
  if (hasAnimations && !hasReducedMotion) {
    violations.push({
      ...
      message:
        'Component has animations but does not respect prefers-reduced-motion',
      file: filePath,
    })
  }
  ...
  if (hasAnimations) {
    ...
    if (!hasFocusAnimations && !hasReducedMotion) {
      warnings.push({
        ...
        message:
          'Non-essential animations should be disabled in reduced motion mode',
        file: filePath,
      })
    }
  }
...
function validateMotionTokens(filePath, fileContent, rules) {
  ...
  const hasMotionTokens =
    fileContent.includes('--motion-duration-') ||
...
function validateAnimationGuardrails(filePath, fileContent, rules) {
    ...
    if (pattern.includes('top/left/width/height')) {
      ...
      if (layoutProps.test(fileContent)) {
        violations.push({
          ...
          message:
            'Animating layout properties (top/left/width/height) causes performance issues. Use transform instead.',
          file: filePath,
        })
      }
    }
  ...
  if (required.includes('Use transform instead of layout properties')) {
    const hasTransform =
      fileContent.includes('transform:') ||
      fileContent.includes('translate') ||
      fileContent.includes('scale') ||
      fileContent.includes('rotate')

    const hasLayoutAnimation =
      /(?:top|left|right|bottom|width|height)[:\s]+(?:var|calc)/gi.test(
        fileContent
      )
