...
export async function validateComponent(filePath, fileContent = null) {
  ...
  // Styling validation
  const stylingResult = await validateComponentStyling(
    filePath,
    fileContent,
    rules
  )
...
function validateComponentStructure(ast, rules, filePath) {
  ...
  if (structureRules.includes('forwardRef for components that accept refs')) {
    ...
    const hasRef =
      filePath.includes('ref') || ast.toString().includes('React.Ref')
...
async function validateComponentStyling(filePath, fileContent, rules) {
  ...
  for (const pattern of forbidden) {
    if (
      pattern.includes('hex colors') &&
      /#[0-9a-fA-F]{3,6}/i.test(fileContent)
    ) {
      violations.push({
        rule: 'COMP-STYLE-001',
    ...
    if (pattern.includes('tailwind palette colors')) {
      const paletteRegex =
        /(?:bg|text|border)-(?:red|blue|green|yellow|orange|purple|pink|gray)-\d+/g
      if (paletteRegex.test(fileContent)) {
        violations.push({
          rule: 'COMP-STYLE-002',
          type: 'palette_color_usage',
          message:
            'Component uses Tailwind palette colors (use tokens instead)',
          file: filePath,
        })
      }
    }
...
export function detectComponentCategory(filePath, fileContent) {
  ...
  const hasRecharts = imports.some(
    imp => imp.source.includes('recharts') || imp.source.includes('visx')
  )
