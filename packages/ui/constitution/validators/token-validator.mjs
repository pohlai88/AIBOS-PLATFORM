...
export async function validateToken(tokenName, filePath, options = {}) {
  ...
  if (tokenName.includes("-hover") || tokenName.includes("-active") || 
      tokenName.includes("-disabled") || tokenName.includes("-selected")) {
...
function validateCrossCategoryConflicts(tokenName, rules) {
  ...
  for (const conflictRule of conflictRules) {
    ...
    if (tokenName.startsWith("--color-") && rule.includes("spacing")) {
      if (tokenName.includes("space") || tokenName.includes("gap") || 
          tokenName.includes("padding") || tokenName.includes("margin")) {
        return {
          valid: false,
...
export async function validateTokensInFile(filePath, fileContent) {
  ...
  const tailwindTokenRegex = /(?:bg|text|border|ring|shadow)-\[var\((--[a-z0-9-]+)\)\]/gi;
