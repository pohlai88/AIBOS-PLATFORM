...
export async function validateRSCBoundaries(filePath, fileContent = null) {
  ...
  if (!isClientComponent) {
    ...
    // Check for hydration
    if (
      fileContent.includes('ReactDOM.render') ||
    ...
    if (!importChainResult.valid) {
      violations.push(
        ...importChainResult.violations.map(v => ({
          rule: 'RSC-006',
  ...
  if (forbidden.includes('inline styles (dynamic)')) {
    ...
    if (dynamicStyleRegex.test(fileContent)) {
      violations.push({
        ...
        message:
          'Server component uses dynamic inline styles (use CSS variables instead)',
      })
    }
  }
