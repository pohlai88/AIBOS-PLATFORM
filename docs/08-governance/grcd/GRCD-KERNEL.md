...
> The Kernel is not a generic platform. It is the constitutional
> control plane for an AI-governed ERP / Business Operating System.
...
**ERP Universe & Kernel Boundaries**

- **Kernel governs:**
  - Cross-module governance for **COA, party, item, dimension, and workflows**.

  - AI orchestras for finance (GL, consolidation, forecasts), compliance

    (MFRS/IFRS, tax, grants), and operations (inventory, production, sales).

  - Tenant, company, and jurisdiction isolation at the governance level.

- **Kernel DOES NOT:**
  - Store ledger lines, invoices, stock movements, or transactional rows.

  - Render ERP UI screens for GL, AR, AP, Inventory, etc.

  - Run batch jobs (posting, accrual, depreciation) directly.

  - Replace any downstream ERP engine (it governs them via contracts).
...
| F-ERP-1 | Kernel MUST enforce IFRS/MFRS as canonical financial standard for all finance-related flows. | MUST                       | ⬜                   | All other accounting views are derived via metadata mappings.          |
| F-ERP-2 | Kernel MUST support multi-company, multi-entity, multi-currency ERP configurations.          | MUST                       | ⬜                   | Entity + currency are mandatory context in finance orchestras.         |
| F-ERP-3 | Kernel MUST govern shared ERP catalogs (COA, party, item, tax, dimensions) as MDM.           | MUST                       | ⬜                   | No local/private COA or party tables allowed in downstream apps.       |
| F-ERP-4 | Kernel MUST allow domain-specific "industry packs" via manifests (Farming, F&B, Retail).     | MUST                       | ⬜                   | Implemented as manifest bundles + feature flags per tenant.            |
| F-ERP-5 | Kernel SHOULD orchestrate ERP workflows (e.g., P2P, O2C, R2R) as versioned orchestration.    | SHOULD                     | ⬜                   | Workflows are expressed as orchestration manifests with HITL steps.    |
...
> AI-BOS ERP is modeled as a **molecular system** governed by the Kernel.

> Atoms = master data, Molecules = functional cores, Organisms = suites,

> Orchestras = AI-assisted cross-suite workflows.

#### 3.4.1 Molecular Layering

| Layer     | Meaning                                  | Examples (AI-BOS)                             |
| --------- | ---------------------------------------- | --------------------------------------------- |
| Atom      | Primitive master data / shared construct | `ATOM_COA_ACCOUNT`, `ATOM_PARTY`, `ATOM_ITEM` |
| Molecule  | Reusable functional cluster              | `MOL_GL_CORE`, `MOL_AR_CORE`, `MOL_INVENTORY` |
| Organism  | End-user ERP app / vertical solution     | `ORG_FINANCIALS_SUITE`, `ORG_FARMING_SUITE`   |
| Orchestra | AI-assisted cross-organism workflow      | `ORCH_PROCURE_TO_PAY`, `ORCH_PERIOD_CLOSE`    |

#### 3.4.2 ERP Component Catalog (AI-BOS v1)

| Component ID         | Layer     | Domain      | Responsibility Boundary                                                             | Kernel Contract Touchpoints                                 |
| -------------------- | --------- | ----------- | ----------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| ATOM_COA_ACCOUNT     | Atom      | Finance     | Canonical chart of accounts, account types, normal balance, disclosures, tax flags. | `mdm_coa` manifest, validation MCP, reporting orchestras.   |
| ATOM_PARTY           | Atom      | Shared      | Legal entities, customers, suppliers, staff, franchisees, grant bodies.             | Identity, credit policy, KYC/AML checks, tenancy mapping.   |
| ATOM_ITEM            | Atom      | Operations  | SKUs for agri, F&B, retail, services; units, tax groups, costing method.            | Pricing, tax, inventory, COGS posting contracts.            |
| ATOM_LOCATION        | Atom      | Operations  | Farms, outlets, warehouses, central kitchen, digital nodes.                         | Stock, sales, and production dimensionality.                |
| MOL_GL_CORE          | Molecule  | Finance     | Journal engine, postings, trial balance, period closing, consolidation hooks.       | Posting contracts, policy engine, audit trail integration.  |
| MOL_AR_CORE          | Molecule  | Finance     | Invoicing, receipts, credit notes, ageing, credit control.                          | Party exposure limits, revenue recognition rules.           |
| MOL_AP_CORE          | Molecule  | Finance     | Supplier invoices, 3-way match (PO→GRN→Invoice), payments.                          | Procurement policies, cashflow, commitment accounting.      |
| MOL_INVENTORY_CORE   | Molecule  | Ops/SCM     | Stock ledger, movements, costing, shrinkage, stock takes.                           | GL posting contracts, cost adjustment policies.             |
| MOL_PROJECT_COSTING  | Molecule  | Finance/Ops | Project / crop / program costing, WIP tracking, grants allocation.                  | MFRS/IFRS project capitalization vs expense decisions.      |
| ORG_FINANCIALS_SUITE | Organism  | Finance     | CFO workspace: FS, dashboards, consolidation, covenant monitoring.                  | Kernel-governed query contracts, disclosure packs.          |
| ORG_FARMING_SUITE    | Organism  | Farming     | Crop cycles, yields, biological assets, farming costs, grants.                      | Domain-specific manifests aligned to MFRS/IFRS treatment.   |
| ORCH_PROCURE_TO_PAY  | Orchestra | Cross-ERP   | PO→GRN→Invoice→Payment with AI assistance and approvals.                            | Policy engine, MCP orchestration, audit & HITL checkpoints. |
| ORCH_PERIOD_CLOSE    | Orchestra | Finance     | Pre-close checks, accruals, reconciliations, reporting sign-off.                    | Compliance orchestra, Franchise/Group reporting.            |

> Over time you can expand this table but keep IDs stable.

> When a component gets its own GRCD, it should reference its ID above.
...
9. Agent-related files MUST go in `kernel/agents/` subdirectories.
10. Observability files MUST go in `kernel/observability/` subdirectories.
11. Governance files (HITL) MUST go in `kernel/governance/` subdirectories.
12. Finance compliance files MUST go in `kernel/finance/` subdirectories.
13. Distributed features MUST go in `kernel/distributed/` subdirectories.
...
#### 5.x ERP Component Dependency Notes (Logical, Not Code Imports)

> These are **logical** dependencies between ERP molecules.

> They are enforced via manifests + MCP contracts, not tight code imports.

- `MOL_GL_CORE`:
  - Depends on: `ATOM_COA_ACCOUNT`, `ATOM_PARTY`, `ATOM_CURRENCY`.

  - Provides: canonical postings, balances, and consolidation hooks.

- `MOL_AR_CORE`:
  - Depends on: `MOL_GL_CORE`, `ATOM_PARTY`, `ATOM_ITEM`.

  - Provides: receivables events and revenue recognition inputs.

- `MOL_INVENTORY_CORE`:
  - Depends on: `ATOM_ITEM`, `ATOM_LOCATION`, `MOL_GL_CORE`.

  - Provides: cost of sales and stock valuation adjustments.
...
- `K-MCP-4`: MCP MUST reference the current GRCD version (**v4.1.0**).
...
> ✅ **Status:** GRCD-KERNEL has been created as **v4.1.0**, fully aligned with the **GRCD Template v4**, **AI-BOS Nexus Whitepaper (Zero Drift Governance Framework)**, and **AI-Orchestra Ecosystem Strategy**. This document is the **single source of truth** for the Kernel.

---

## Changelog

### Version 4.1.0 (2025-01-27)

**Added:**

- **Section 1.4 - ERP Context**: Added comprehensive ERP suite context (ZohoONE-inspired unified suite), standards & source-of-truth (IFRS/MFRS canonical, domain catalogs, alias system), and ERP universe & kernel boundaries
- **Section 3.4 - ERP Component Map (Molecular Design)**: Added molecular layering framework (Atom → Molecule → Organism → Orchestra) and ERP Component Catalog with 12 components (4 Atoms, 5 Molecules, 2 Organisms, 2 Orchestras)
- **Section 2.1 - ERP Functional Requirements**: Added 5 new ERP-specific requirements (F-ERP-1 through F-ERP-5) covering IFRS/MFRS enforcement, multi-company/entity/currency support, shared ERP catalogs governance, industry packs, and ERP workflow orchestration
- **Section 5.x - ERP Component Dependencies**: Added logical dependency notes for ERP molecules (MOL_GL_CORE, MOL_AR_CORE, MOL_INVENTORY_CORE)

**Changed:**

- Updated document to reflect ERP-first architecture with molecular design patterns
- Enhanced requirements table with ERP-specific functional requirements

**Impact:**

- Kernel GRCD now explicitly documents ERP context and molecular component architecture
- Provides foundation for other ERP components to reference their Component IDs
- Establishes clear boundaries between Kernel governance and downstream ERP engines

### Version 4.0.0 (2025-11-29)

**Initial Release:**

- Created GRCD-KERNEL v4.0.0 with full GRCD Template v4 compliance
- Established MCP governance framework
- Defined AI-Orchestra coordination patterns
- Set up 8 domain orchestras (DB, UX, BFF, Backend, Compliance, Observability, Finance, DevEx)
