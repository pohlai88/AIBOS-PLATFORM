 ...
 * @returns {Object} Validation result
 */
export async function validateVisual(filePath, fileContent = null, options = {}) {
  ...
  const snapshotResult = validateSnapshotBaseline(filePath, visualRules, options);
...
function validateSnapshotBaseline(filePath, rules, options) {
  ...
  const snapshotDir = options.snapshotDir || path.join(
  ...
  if (required.includes("Snapshot of all variants")) {
    ...
    const hasVariants = fileContent.includes("variant=") ||
                       fileContent.includes("variants") ||
                       fileContent.includes("variant:");

    if (hasVariants) {
      // Check for variant-specific snapshots
      const variantSnapshotPattern = path.join(snapshotDir, `${componentName}.*.snap`);
  ...
  if (required.includes("Snapshot of safe mode")) {
    const safeModeSnapshot = path.join(snapshotDir, `${componentName}.safe-mode.snap`);
  ...
  if (required.includes("Snapshot of reduced motion")) {
    const reducedMotionSnapshot = path.join(snapshotDir, `${componentName}.reduced-motion.snap`);
...
function validateAutoRollback(filePath, rules, options) {
  ...
  if (rollbackRules.rules) {
    ...
    if (options.visualDiff && options.visualDiff.diff > 0) {
      ...
      if (diff > threshold && rollbackRules.rules.includes("Rollback on unacceptable deviation")) {
        warnings.push({
          rule: "VISUAL-007",
...
export async function captureSnapshot(filePath, options = {}) {
  ...
  return {
    success: true,
    message: "Snapshot capture should be done via mcp_visual_regression_capture",
    filePath,
  };
}
...
export async function compareSnapshot(filePath, options = {}) {
  ...
  return {
    success: true,
    message: "Snapshot comparison should be done via mcp_visual_regression_compare",
    filePath,
    diff: 0,
