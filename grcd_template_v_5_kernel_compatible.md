# 🧾 GRCD — Template v5 (MCP‑Governed + ERP Edition)

**Version:** 5.0.0  
**Status:** Active (MCP‑Governed + ERP Template & Reference)  
...
### 1.4 ERP Context (if applicable)

> Use this section when the component lives inside an ERP / business suite.

> Anchor the component in that ERP reality (domains, standards, tenants, industries).

**ERP Suite Context**

- **Suite Name:** `{{ERP_SUITE_NAME}}` (e.g., `AI-BOS ERP`, `Nexus ERP Suite`)

- **Inspiration / Model:** `{{ERP_INSPIRATION}}` (e.g., "ZohoONE-style unified suite")

- **Scope of Suite:**
  - `{{ERP_SCOPE}}` (e.g., "Multi-company, multi-entity, multi-currency, multi-industry")

**Standards & Source-of-Truth**

- **Financial SSOT:** `{{ERP_FINANCIAL_SSOT}}`
  - Example: `IFRS/MFRS` as canonical, `local GAAP` as alias via metadata.

- **Domain Catalogs:**
  - Finance: `{{ERP_FINANCE_METADATA}}` (COA, dimensions, parties, tax, disclosures)

  - Operations: `{{ERP_OPS_METADATA}}` (items/SKUs, locations, channels, projects)

- **Alias System (Semantic Normalization):**
  - Example: `Revenue ≈ Income ≈ Gain` (IFRS "Revenue" = canonical; others = aliases)

  - Example: `Apple / APPLE / apples` → one canonical entity with metadata aliases.

**ERP Universe & Boundaries for this Component**

- **This component governs:**
  - `{{ERP_GOVERNED_AREAS}}` (e.g., "cross-module governance for COA, party, item, workflows")

- **This component explicitly does NOT:**
  - `{{ERP_NON_RESPONSIBILITY}}` (e.g., "render screens", "store tenant ledger lines", "run batch jobs")

> **Template Note:** For non-ERP components, omit this section entirely or mark as N/A.
...
| F-ERP-1 | Component MUST respect IFRS/MFRS as canonical financial standard (if finance-related).     | MUST                       | ⬜                   | Local GAAP mapped as metadata aliases.         |
| F-ERP-2 | Component MUST support multi-company, multi-entity, multi-currency setups (if applicable). | MUST                       | ⬜                   | Entity + currency as mandatory dimensions.     |
| F-ERP-3 | Component MUST consume COA, party, and item catalogs from MDM, not local (if applicable).  | MUST                       | ⬜                   | No private COA/party/item tables allowed.      |
| F-ERP-4 | Component SHOULD support industry packs as pluggable manifest bundles (if applicable).     | SHOULD                     | ⬜                   | e.g., Farming, F&B, Retail, Franchise.         |
...
### 3.4 ERP Component Map (Molecular Design) (if applicable)

> Use this to describe the ERP landscape as **atoms → molecules → organisms → orchestras**.

> The same pattern applies across all ERP components.

#### 3.4.1 Molecular Layering (Shared Vocabulary)

| Layer     | Meaning                                  | Examples                                      |
| --------- | ---------------------------------------- | --------------------------------------------- |
| Atom      | Primitive master data / reference object | `ATOM_COA_ACCOUNT`, `ATOM_PARTY`, `ATOM_ITEM` |
...
#### 3.4.2 ERP Component Catalog (Template)

| Component ID       | Layer     | Domain     | Responsibility Boundary                   | Kernel Contract Touchpoints                           |
| ------------------ | --------- | ---------- | ----------------------------------------- | ----------------------------------------------------- |
| {{COMPONENT_ID_1}} | Atom      | {{DOMAIN}} | {{WHAT_IT_OWNS}}                          | {{WHICH_MANIFESTS / MCP / AUDIT IT TOUCHES}}          |
| {{COMPONENT_ID_2}} | Molecule  | {{DOMAIN}} | {{WHAT_IT_DOES}}                          | {{KERNEL_POLICY / EVENTS / CONTRACTS}}                |
| {{COMPONENT_ID_3}} | Organism  | {{DOMAIN}} | {{WHAT_USER_SEES}}                        | {{API, UI contracts, orchestration entry points}}     |
| {{COMPONENT_ID_4}} | Orchestra | Cross-ERP  | {{END-TO-END FLOW (e.g., P2P, O2C, R2R)}} | {{Which molecules it coordinates + HITL checkpoints}} |

> **Guidelines:**

> - Keep `Component ID` stable; treat it like an RFC name.

> - If a component has its own GRCD, reference that ID in its Section 1.2 Identity.

> - Never describe UI/DB here – only **responsibility boundaries** and **contracts**.

> - For non-ERP components, omit this section or mark as N/A.

> **Template Note:** When instantiating this template for an ERP component, replace placeholders with actual component IDs from the master ERP Component Catalog (see GRCD-KERNEL.md Section 3.4.2).
...
#### 5.x ERP Component Dependency Notes (if applicable)

> These are **logical** dependencies between ERP molecules.

> They are enforced via manifests + MCP contracts, not tight code imports.

- `{{MOL_COMPONENT_1}}` depends on: `{{ATOM_COMPONENT_1}}`, `{{ATOM_COMPONENT_2}}`.

- `{{MOL_COMPONENT_2}}` depends on: `{{MOL_COMPONENT_1}}`, `{{ATOM_COMPONENT_3}}`.

> **Template Note:** For non-ERP components, omit this subsection. For ERP components, list logical dependencies (not code imports) referencing Component IDs from the ERP Component Catalog.
...
- `K-MCP-4`: MCP MUST reference the current GRCD version for the component (**v5.0.0**).
...
```json
{
  ...
  "identity": {
    "roles": [
      ...
      {
        "name": "operator",
        "privileges": ["action.execute", "metadata.read", "mcp.tool.invoke"]
      }
    ]
  },
  "audit": {
...
> ✅ **Status:** GRCD‑TEMPLATE has been upgraded to **v5.0.0** and is now structurally compatible with **GRCD‑KERNEL.md v4.1.0 (MCP‑Governed + ERP Edition)**. Use this as the master pattern for future copies and per‑component GRCDs, including ERP components with molecular design patterns.

---

## Changelog

### Version 5.0.0 (2025-01-27)

**Added:**

- **Section 1.4 - ERP Context**: Added template section for ERP suite context, standards & source-of-truth, and ERP universe boundaries (with placeholders for customization)
- **Section 3.4 - ERP Component Map (Molecular Design)**: Added molecular layering framework and ERP Component Catalog template with placeholders
- **Section 2.1 - ERP Functional Requirements**: Added 4 ERP-specific requirement rows (F-ERP-1 through F-ERP-4) as template examples
- **Section 5.x - ERP Component Dependencies**: Added template subsection for logical ERP component dependencies

**Changed:**

- Updated template version from v4.0.0 to v5.0.0
- Enhanced template to support both generic components and ERP-specific components
- Added guidance notes for when to include/omit ERP sections

**Impact:**

- Template now supports creating GRCDs for ERP components (Atoms, Molecules, Organisms, Orchestras)
- Provides clear placeholders and examples for ERP context integration
- Maintains backward compatibility for non-ERP components

### Version 4.0.0 (2025-11-28)

**Initial v4 Release:**

- Created MCP-Governed template aligned with GRCD-KERNEL v4.0.0
- Added MCP governance sections and schemas
- Established template structure for AI-BOS Nexus components
